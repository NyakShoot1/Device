...
TARGET_BOARD_PLATFORM := msm8998 # определяет платформу, на которой
построено устройство (зависит от используемого процессора)
# Architecture
TARGET_ARCH := arm64 # основная архитектура процессора
TARGET_ARCH_VARIANT := armv8-a # указывает на вариант архитектуры
armv8-a
TARGET_CPU_ABI := arm64-v8a # Application Binary Interface
(https://developer.android.com/ndk/guides/abis набор поддерживаемых
процессором команд)
TARGET_CPU_ABI2 :=
...
процессором
TARGET_2ND_ARCH_VARIANT := armv8-a # указывает на вариант архитектуры
armv8-a
TARGET_2ND_CPU_ABI := armeabi-v7a # Application Binary Interface
(https://developer.android.com/ndk/guides/abis набор поддерживаемых
процессором команд)
...
# Kernel
TARGET_KERNEL_SOURCE := kernel/htc/msm8998 # указывает путь к исходному
коду ядра.
TARGET_KERNEL_CONFIG := lineage_ocn_defconfig # указывает имя
конфигурационного файла для сборки ядра.
...
в байтах
BOARD_SYSTEMIMAGE_PARTITION_SIZE := 314572800 # размер образа системы в
байтах
...
BOARD_USES_RECOVERY_AS_BOOT := true # определение, что recovery
встроено в ядро
